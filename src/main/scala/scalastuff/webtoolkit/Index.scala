...
class Index(file : String = "", path : List[String] = Nil, val inheritedProcessors : Seq[Processor] = Seq())(implicit var context : Context) extends Logging {
	...
	// load the index file
	val xml : Elem = try {XML.load(CurrentTemplateURI.get.openStream)} catch { case e => Throw(e) }
	...
	def processors(parent : Node, inheritedOnly : Boolean = false) = parent.child.collect {
		case e : Elem if e.label == "processor" && (!inheritedOnly || e ?? "@inherit")  => 
			AvailableProcessors.find(_.name == Some(e get "@name")).
				getOrElse(Throw("Processor '" + e.get("@name") + "' not found"))
	}
		...
		xml.child.collect {
			case e : Elem if e.label == "page" => 
				new FileBasedPage(
					...
					decorators(xml) ++ decorators(e) ++ processors(xml) ++ processors(e) ++ inheritedProcessors ++ DefaultProcessors)
		}
